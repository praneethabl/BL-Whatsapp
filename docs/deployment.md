...
access key ID at `/opt/blwhatsappcopy/secrets/r2_media_access_key_id` and its secret
at `/opt/blwhatsappcopy/secrets/r2_media_secret_access_key`, both mode `0640` and owned by the deployment user's secrets group. Set
...
`/run/blwhatsappcopy-control/http-bearer-token` on a root-owned `0700` tmpfs, with
...
`blwhatsappcopy_worker_runtime` grant role; the file-only
`worker-credential-provisioner` creates or rotates the `blwhatsappcopy_worker`
...
`/var/lib/blwhatsappcopy/worker-artifacts/<version>/whatsapp-worker` and its sibling
...
docker build --target artifact-installer -f services/whatsapp/Dockerfile \
  -t blwhatsappcopy/whatsapp-worker-artifact:check .
docker build --target worker -f services/whatsapp/Dockerfile \
  -t blwhatsappcopy/worker:check .
```
...
$COMPOSE exec -T -u 0 \
  -e TARGET_VERSION="$WORKER_ARTIFACT_VERSION" orchestrator sh -ec '
    token=$(cat /run/blwhatsappcopy-control/http-bearer-token)
    digest=$(cat "/var/lib/blwhatsappcopy/worker-artifacts/$TARGET_VERSION/sha256")
