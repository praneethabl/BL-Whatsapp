...
```dotenv
WORKER_POSTGRES_PASSWORD_FILE=/opt/blwhatsappcopy/secrets/worker_postgres_password
NATS_SERVICE_PASSWORD_FILE=/opt/blwhatsappcopy/secrets/nats_service_password
NATS_WORKER_PASSWORD_FILE=/opt/blwhatsappcopy/secrets/nats_worker_password
```
...
5. Apply migration 072. It creates only the NOLOGIN
   `blwhatsappcopy_worker_runtime` grant role; it contains no password.
6. Run `worker-credential-provisioner`. It creates/rotates the
   `blwhatsappcopy_worker` login from the mounted file without putting the password
...
- `blwhatsappcopy_worker` can perform DML only in `whatsapp_sessions`, including
