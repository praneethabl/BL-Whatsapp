...
func TestDurableManagerRequiresDistinctRestrictedWorkerCredentials(t *testing.T) {
	for _, testCase := range []struct {
		...
		{name: "wrong nats user", cfg: Config{DatabaseURL: "manager", WorkerDatabaseURL: "postgresql://blwhatsappcopy_worker:secret@db/app", WorkerNATSURL: "nats://service:secret@nats", NodeID: "node-1"}, want: "dedicated"},
	} {
...
func TestRestrictedWorkerCredentialsAllowSingleHostAndPerNodeIdentities(t *testing.T) {
	for _, testCase := range []struct {
		...
		{name: "single-host database", url: "postgresql://blwhatsappcopy_worker:secret@db/app", scheme: "postgresql", singleHost: "blwhatsappcopy_worker"},
		{name: "per-node database", url: "postgresql://wti_w_0123456789abcdefabcd:secret@db/app", scheme: "postgresql", singleHost: "blwhatsappcopy_worker"},
		...
		{name: "uppercase suffix", url: "postgresql://wti_w_0123456789abcdefabcD:secret@db/app", scheme: "postgresql", singleHost: "blwhatsappcopy_worker", wantError: true},
		{name: "short suffix", url: "nats://wti-w-0123456789abcdefabc:secret@nats", scheme: "nats", singleHost: "worker", wantError: true},
		{name: "wrong prefix", url: "postgresql://wti_m_0123456789abcdefabcd:secret@db/app", scheme: "postgresql", singleHost: "blwhatsappcopy_worker", wantError: true},
	} {
		t.Run(testCase.name, func(t *testing.T) {
