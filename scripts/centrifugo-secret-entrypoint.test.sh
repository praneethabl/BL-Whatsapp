...
docker build -f "$ROOT/infrastructure/centrifugo/Dockerfile" \
  -t blwhatsappcopy/centrifugo:v6.9.1-redacted "$ROOT" >/dev/null
if [[ $(docker image inspect blwhatsappcopy/centrifugo:v6.9.1-redacted --format '{{.Config.User}}') != centrifugo ]]; then
...
run_wrapper() {
  ...
  docker run -d --name "$name" \
    ...
    blwhatsappcopy/centrifugo:v6.9.1-redacted /usr/local/bin/test-child "$@" >/dev/null
}
...
for full_case in high-success:74 high-failure:23; do
  ...
  docker run -d --name "$full_container" \
    ...
    -v "$tmp/token:/run/test-secrets/token:ro" \
    -v "$tmp/api:/run/test-secrets/api:ro" \
    blwhatsappcopy/centrifugo:v6.9.1-redacted -c \
...
for pipe_case in \
  ...
  docker run -d --name "$pipe_container" \
    ...
    -v "$tmp/token:/run/test-secrets/token:ro" \
    -v "$tmp/api:/run/test-secrets/api:ro" \
    blwhatsappcopy/centrifugo:v6.9.1-redacted -o pipefail -c "$pipeline" \
    wrapper-test "$mode" >/dev/null
