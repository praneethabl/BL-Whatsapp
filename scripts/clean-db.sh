...
VOLUMES=(
    "blwhatsappcopy_postgres_data"
    "blwhatsappcopy_nats_data"
    "blwhatsappcopy_meilisearch_data"
    "blwhatsappcopy_minio_data"
)

# Container names
CONTAINERS=(
    "blwhatsappcopy-postgres"
    "blwhatsappcopy-nats"
    "blwhatsappcopy-meilisearch"
    "blwhatsappcopy-minio"
    "blwhatsappcopy-minio-init"
)
...
start_fresh() {
    ...
    sleep 3
    for i in {1..30}; do
        if docker exec blwhatsappcopy-postgres pg_isready -U postgres >/dev/null 2>&1; then
