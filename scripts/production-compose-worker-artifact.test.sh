...
jq -e '
  .services["worker-artifact-installer"].image == "blwhatsappcopy/whatsapp-worker-artifact:2026-08-02.1" and
  ...
  .services.orchestrator.environment.WHATSAPP_BINARY_PATH == "/var/lib/blwhatsappcopy/worker-artifacts/bootstrap/whatsapp-worker" and
  .services.orchestrator.environment.EPHEMERAL_HTTP_BEARER_TOKEN_FILE == "/run/blwhatsappcopy-control/http-bearer-token" and
  ...
  any(.services.orchestrator.tmpfs[]; . == "/run/blwhatsappcopy-control:mode=0700,uid=0,gid=0") and
  .services.orchestrator.environment.WORKER_ARTIFACT_ROOT == "/var/lib/blwhatsappcopy/worker-artifacts" and
  .services.orchestrator.environment.WORKER_DEFAULT_ARTIFACT_VERSION == "bootstrap" and
  any(.services.orchestrator.volumes[]; .target == "/var/lib/blwhatsappcopy/worker-artifacts" and .read_only == true) and
  any(.services["worker-artifact-installer"].volumes[]; .target == "/var/lib/blwhatsappcopy/worker-artifacts" and (.read_only != true))
...
jq -e '
  .services["worker-artifact-installer"].image == "blwhatsappcopy/whatsapp-worker-artifact:worker-2026.08.03" and
  .services["worker-artifact-installer"].environment.WORKER_ARTIFACT_VERSION == "worker-build-a1" and
  .services.orchestrator.environment.WORKER_DEFAULT_ARTIFACT_VERSION == "bootstrap" and
  .services.orchestrator.environment.WHATSAPP_BINARY_PATH == "/var/lib/blwhatsappcopy/worker-artifacts/bootstrap/whatsapp-worker"
