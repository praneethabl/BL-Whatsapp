...
docker run -d --name "$database" --network "$network" --network-alias postgres \
  -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD="$admin_password" -e POSTGRES_DB=blwhatsappcopy \
  postgres:18.1-alpine >/dev/null
for _ in $(seq 1 100); do
  if docker exec "$database" psql -U postgres -d blwhatsappcopy -Atc 'SELECT 1' >/dev/null 2>&1; then break; fi
  sleep 0.1
done
docker exec "$database" psql -U postgres -d blwhatsappcopy -Atc 'SELECT 1' >/dev/null
docker exec -i "$database" psql -U postgres -d blwhatsappcopy -v ON_ERROR_STOP=1 <<'SQL' >/dev/null
REVOKE CREATE ON SCHEMA public FROM PUBLIC;
CREATE ROLE blwhatsappcopy_worker_runtime NOLOGIN INHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS;
GRANT CONNECT ON DATABASE blwhatsappcopy TO blwhatsappcopy_worker_runtime;
...
GRANT USAGE ON SCHEMA whatsapp_sessions TO blwhatsappcopy_worker_runtime;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA whatsapp_sessions TO blwhatsappcopy_worker_runtime;
GRANT USAGE, SELECT, UPDATE ON ALL SEQUENCES IN SCHEMA whatsapp_sessions TO blwhatsappcopy_worker_runtime;
ALTER DEFAULT PRIVILEGES IN SCHEMA whatsapp_sessions GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO blwhatsappcopy_worker_runtime;
ALTER DEFAULT PRIVILEGES IN SCHEMA whatsapp_sessions GRANT USAGE, SELECT, UPDATE ON SEQUENCES TO blwhatsappcopy_worker_runtime;
SQL

run_provisioner() {
  docker run --rm --network "$network" \
    -e POSTGRES_USER=postgres -e POSTGRES_DB=blwhatsappcopy \
...
assert_worker_role_absent() {
  local count
  count=$(docker exec "$database" psql -U postgres -d blwhatsappcopy -Atc \
    "SELECT count(*) FROM pg_roles WHERE rolname = 'blwhatsappcopy_worker'")
  [[ $count == 0 ]] || {
...
result=$(docker exec -e PGPASSWORD="$worker_password" "$database" \
  psql -h 127.0.0.1 -U blwhatsappcopy_worker -d blwhatsappcopy -Atc \
  "SELECT current_user, pg_has_role(current_user, 'blwhatsappcopy_worker_runtime', 'member')")
[[ $result == 'blwhatsappcopy_worker|t' ]]
...
admin_sql() {
  docker exec "$database" psql -U postgres -d blwhatsappcopy -v ON_ERROR_STOP=1 -c "$1" >/dev/null
}
...
expect_role_policy_rejected() {
  ...
  current=$(docker exec -e PGPASSWORD="$worker_password" "$database" \
    psql -h 127.0.0.1 -U blwhatsappcopy_worker -d blwhatsappcopy -Atc 'SELECT current_user')
  [[ $current == 'blwhatsappcopy_worker' ]] || {
    echo "rejected $label changed the existing worker login" >&2
    exit 1
  }
}

admin_sql 'ALTER ROLE blwhatsappcopy_worker SUPERUSER'
expect_role_policy_rejected 'a superuser worker role'
admin_sql 'ALTER ROLE blwhatsappcopy_worker NOSUPERUSER'

admin_sql 'ALTER ROLE blwhatsappcopy_worker_runtime BYPASSRLS'
expect_role_policy_rejected 'a BYPASSRLS runtime role'
admin_sql 'ALTER ROLE blwhatsappcopy_worker_runtime NOBYPASSRLS'

admin_sql 'CREATE ROLE worker_forbidden_parent NOLOGIN; GRANT worker_forbidden_parent TO blwhatsappcopy_worker'
expect_role_policy_rejected 'an unexpected role membership'
admin_sql 'REVOKE worker_forbidden_parent FROM blwhatsappcopy_worker; DROP ROLE worker_forbidden_parent'

admin_sql 'GRANT blwhatsappcopy_worker_runtime TO blwhatsappcopy_worker WITH ADMIN OPTION'
expect_role_policy_rejected 'an ADMIN OPTION on the runtime membership'
admin_sql 'REVOKE ADMIN OPTION FOR blwhatsappcopy_worker_runtime FROM blwhatsappcopy_worker'

admin_sql 'CREATE ROLE worker_forbidden_member NOLOGIN; GRANT blwhatsappcopy_worker TO worker_forbidden_member'
expect_role_policy_rejected 'an unexpected member of the worker login role'
admin_sql 'REVOKE blwhatsappcopy_worker FROM worker_forbidden_member; DROP ROLE worker_forbidden_member'

admin_sql 'CREATE TABLE public.worker_owned_object (id integer); ALTER TABLE public.worker_owned_object OWNER TO blwhatsappcopy_worker'
...
admin_sql 'GRANT SELECT ON whatsapp_sessions.device TO blwhatsappcopy_worker'
expect_role_policy_rejected 'a direct worker object grant'
admin_sql 'REVOKE SELECT ON whatsapp_sessions.device FROM blwhatsappcopy_worker'

admin_sql 'GRANT CREATE ON SCHEMA whatsapp_sessions TO blwhatsappcopy_worker'
expect_role_policy_rejected 'worker schema-create authority'
admin_sql 'REVOKE CREATE ON SCHEMA whatsapp_sessions FROM blwhatsappcopy_worker'

admin_sql 'ALTER DEFAULT PRIVILEGES IN SCHEMA whatsapp_sessions GRANT TRUNCATE ON TABLES TO blwhatsappcopy_worker_runtime'
expect_role_policy_rejected 'an unexpected runtime default privilege'
admin_sql 'ALTER DEFAULT PRIVILEGES IN SCHEMA whatsapp_sessions REVOKE TRUNCATE ON TABLES FROM blwhatsappcopy_worker_runtime'

matrix=$(docker exec "$database" psql -U postgres -d blwhatsappcopy -Atc "
  SELECT concat_ws('|',
    (SELECT rolcanlogin AND rolinherit AND NOT rolsuper AND NOT rolcreatedb AND NOT rolcreaterole AND NOT rolreplication AND NOT rolbypassrls FROM pg_roles WHERE rolname='blwhatsappcopy_worker'),
    (SELECT NOT rolcanlogin AND rolinherit AND NOT rolsuper AND NOT rolcreatedb AND NOT rolcreaterole AND NOT rolreplication AND NOT rolbypassrls FROM pg_roles WHERE rolname='blwhatsappcopy_worker_runtime'),
    pg_has_role('blwhatsappcopy_worker', 'blwhatsappcopy_worker_runtime', 'member'),
    has_schema_privilege('blwhatsappcopy_worker', 'whatsapp_sessions', 'USAGE'),
    NOT has_schema_privilege('blwhatsappcopy_worker', 'whatsapp_sessions', 'CREATE'),
    has_table_privilege('blwhatsappcopy_worker', 'whatsapp_sessions.device', 'SELECT,INSERT,UPDATE,DELETE')
  )")
[[ $matrix == 't|t|t|t|t|t' ]]
...
docker exec "$database" psql -U postgres -d blwhatsappcopy -v ON_ERROR_STOP=1 \
