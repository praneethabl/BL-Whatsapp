...
output=$(docker run --rm \
  -e POSTGRES_USER=service -e POSTGRES_DB=blwhatsappcopy \
  ...
  alpine:3.22 /bin/sh /entrypoint sh -ec '
    ...
    case "$WORKER_DATABASE_URL" in postgresql://blwhatsappcopy_worker:WorkerPassword_*@postgres:5432/*) ;; *) exit 1;; esac
