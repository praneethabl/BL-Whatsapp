...
DO $preflight$
DECLARE
  worker_oid oid := (SELECT oid FROM pg_roles WHERE rolname = 'blwhatsappcopy_worker');
  runtime_oid oid := (SELECT oid FROM pg_roles WHERE rolname = 'blwhatsappcopy_worker_runtime');
BEGIN
  IF runtime_oid IS NULL THEN
    RAISE EXCEPTION 'blwhatsappcopy_worker_runtime is missing; apply migration 072 first';
  END IF;

  IF EXISTS (
    SELECT 1 FROM pg_roles
    WHERE rolname = 'blwhatsappcopy_worker_runtime'
      AND (rolcanlogin OR NOT rolinherit OR rolsuper OR rolcreatedb OR rolcreaterole OR rolreplication OR rolbypassrls)
  ) OR EXISTS (
    SELECT 1 FROM pg_roles
    WHERE rolname = 'blwhatsappcopy_worker'
  ...
  IF EXISTS (
    ...
    WHERE has_schema_privilege('blwhatsappcopy_worker_runtime', schema_row.oid, 'CREATE')
       OR (worker_oid IS NOT NULL AND has_schema_privilege('blwhatsappcopy_worker', schema_row.oid, 'CREATE'))
  ) THEN
  ...
  IF worker_oid IS NOT NULL AND EXISTS (
    ...
    RAISE EXCEPTION 'blwhatsappcopy_worker has direct object grants';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'blwhatsappcopy_worker_runtime has grants outside the runtime matrix';
  END IF;
...
DO $block$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'blwhatsappcopy_worker') THEN
    CREATE ROLE blwhatsappcopy_worker LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION;
  END IF;
END
$block$;
ALTER ROLE blwhatsappcopy_worker_runtime NOLOGIN INHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS;
ALTER ROLE blwhatsappcopy_worker LOGIN INHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS;
ALTER ROLE blwhatsappcopy_worker PASSWORD :'worker_password';
ALTER ROLE blwhatsappcopy_worker SET search_path = whatsapp_sessions, pg_catalog;
GRANT blwhatsappcopy_worker_runtime TO blwhatsappcopy_worker;
SQL
