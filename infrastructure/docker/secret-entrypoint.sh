...
if [ -n "${WORKER_POSTGRES_PASSWORD:-}" ] || [ -n "${NATS_WORKER_PASSWORD:-}" ]; then
  ...
  export WORKER_DATABASE_URL="postgresql://blwhatsappcopy_worker:${WORKER_POSTGRES_PASSWORD}@postgres:5432/${POSTGRES_DB}?sslmode=disable"
...
if [ -n "${EPHEMERAL_HTTP_BEARER_TOKEN_FILE:-}" ]; then
  ...
  case "$control_file" in
    /run/blwhatsappcopy-control/*) ;;
    *) echo "secret-entrypoint: control token must be under /run/blwhatsappcopy-control" >&2; exit 1 ;;
  esac
  umask 077
