...
start_docker_services() {
    ...
    until docker exec blwhatsappcopy-postgres pg_isready -U postgres &> /dev/null; do
        sleep 1
    done
...
prepare_worker_boundary() {
    ...
    case "$WORKER_DATABASE_URL" in
        postgresql://blwhatsappcopy_worker:"$WORKER_POSTGRES_PASSWORD"@*) ;;
        *) print_error "WORKER_DATABASE_URL must use the blwhatsappcopy_worker login and WORKER_POSTGRES_PASSWORD"; exit 1 ;;
    esac
    case "$NATS_URL" in
